...
void mkdir ::controller(mkdir *dir)
{
    // Aqui se debe buscar la path en los iscos montados, no en el path del dir
    if (dir->path.empty() || dir->id.empty())
    {
...
int mkdir ::cantidadRutas(string path)
{
    int cantidad = 0;
    char pattern = '/';
    int posInit = 0;
...
string mkdir ::nombre(string path)
{
    char pattern = '/';
    int posInit = 0;
    int posFound = 0;
...
void mkdir ::Createdir(int posInodo, string path, string pathfisc, int posSB, int n, int start)
{
    FILE *file;
    Tabla_Inodos inodo;
    file = fopen(pathfisc.c_str(), "rb+");
                    ...
                    if (mkdr->cantidadRutas(str) > 1)
                    {
                        ...
                        for (int j = 0; j < 4; j++)
                        {
                            if (nm == auxcar.b_content[j].b_name)
                            {
                                ...
                                fclose(file);
                                Createdir(auxcar.b_content[j].b_inodo, str, pathfisc, posSB, n, start);
                                return;
                            }
                    ...
                    else
                    {
                        ...
                        for (int j = 0; j < 4; j++)
                        {
                            if (auxcar.b_content[j].b_inodo < 0)
                            {
                                ...
                                strcpy(Bloque_Aux.padre, splitted.c_str());
                                strcpy(Bloque_Aux.name, nm.c_str());

                                Bloque_Aux.b_content[0].b_inodo = -1;
                                Bloque_Aux.b_content[0].b_name[0] = '0';

                                Bloque_Aux.b_content[1].b_inodo = -1;
                                Bloque_Aux.b_content[1].b_name[0] = '0';

                                Bloque_Aux.b_content[2].b_inodo = -1;
                                Bloque_Aux.b_content[2].b_name[0] = '0';

                                Bloque_Aux.b_content[3].b_inodo = -1;
                                Bloque_Aux.b_content[3].b_name[0] = '0';
                                superbloque.s_first_blo++;
                                ...
                                fwrite(&Bloque_Aux, sizeof(Bloque_Carpetas), 1, file);
                                fclose(file);
...
void mkdir ::create(mkdir *dir)
{
    ...
    if (cantidadRutas(dir->path) > 1)
    {
        ...
        Createdir(superbloque.s_inode_start, dir->path, discoaux.disk_path, start, n, start);
    }
    else
    {
        // Aqui se escribe desde la raiz
        CreateinRoot(superbloque.s_inode_start, dir->path, discoaux.disk_path, start, n, start);
    }
}